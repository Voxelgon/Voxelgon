...
namespace Voxelgon.Geometry {
    public static class GeometryVG {
        ...
        public static Vector2 LineIntersection2D(Vector2 p1, Vector2 d1, Vector2 p2, Vector2 d2) {
        ...
        public static int TriangleWindingOrder2D(Vector2 p0, Vector2 p1, Vector2 p2) {
            return VectorWindingOrder2D(p1 - p0, p2 - p0);
        }
            ...
            while (index3 < vertices.Length && index3 >= 0) {
                ...
                if (GeometryVG.TriangleWindingOrder2D(vertices[index1], vertices[index2], vertices[index3]) == 1) {
                    for (int i = index3 + 1; i < vertices.Length && validTri; i++) {
                        validTri &= !GeometryVG.TriangleContains2D(vertices[index1], vertices[index2], vertices[index3],
                            vertices[i]);
                    }
                }
                else {
                ...
                if (validTri) {
                    ...
                    if (index1 != 0 &&
                        GeometryVG.TriangleWindingOrder2D(vertices[0], vertices[index1], vertices[index3]) == 1) {
                        return index3;
                    }
